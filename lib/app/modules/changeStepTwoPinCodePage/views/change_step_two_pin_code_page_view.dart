...
class ChangeStepTwoPinCodePageView extends StatefulWidget {
  const ChangeStepTwoPinCodePageView({super.key});

  @override
  _ChangeStepTwoPinCodePageViewState createState() => _ChangeStepTwoPinCodePageViewState();
}

class _ChangeStepTwoPinCodePageViewState extends State<ChangeStepTwoPinCodePageView> {
  ...
  void _onNumberTap(String number) {
    setState(() {
      if (_controller.updatedConfirmPinCode.value.length < _pinLength) {
        _controller.updatedConfirmPinCode.value =
            _controller.updatedConfirmPinCode.value + number;
      }
    });
  }
  ...
  void _onDeleteTap() {
    setState(() {
      if (_controller.updatedConfirmPinCode.value.isNotEmpty) {
        _controller.updatedConfirmPinCode.value = _controller.updatedConfirmPinCode.value
            .substring(0, _controller.updatedConfirmPinCode.value.length - 1);
      }
    });
  }
      ...
      children: List.generate(
        _pinLength,
          ...
          decoration: BoxDecoration(
            ...
            color:
            index < _controller.updatedConfirmPinCode.value.length
                ? Colors.blue
                : Colors.grey[300],
          ),
        ),
